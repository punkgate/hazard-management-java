...
public class AdminDashboardFrame extends JFrame {
    ...
    private Color criticalSeverityColor = new Color(139, 0, 0); // dark red
    private Color highSeverityColor = new Color(255, 140, 0); // light orange
    private Color mediumSeverityColor = new Color(255, 215, 0); // yellow
    private Color lowSeverityColor = new Color(76, 175, 80); // green

    public AdminDashboardFrame(String username) {
    setTitle("Admin Dashboard - Hazard Tracker");
        setSize(1200, 800);
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            accidentsTable = new JTable(accidentsModel) {
                ...
                public boolean isCellEditable(int row, int col) {
                    return getColumnName(col).equals("status");
                }
            };
            
            // Add status column renderer
            TableColumn statusColumn = accidentsTable.getColumn("status");
            ...
            accidentsModel.addTableModelListener(evt -> {
                if (evt.getColumn() == accidentsTable.getColumn("status").getModelIndex()) {
                    int row = evt.getFirstRow();
                    String newStatus = (String) accidentsModel.getValueAt(row, evt.getColumn());
                    int accidentId = (int) accidentsModel.getValueAt(row, 0);
                    
                    updateAccidentStatus(accidentId, newStatus);
                }
            });
    ...
    // Create and add analytics panel
    createAnalyticsPanel();

    // Add tabs
    tabbedPane.addTab("Hazard Reports", accidentsPanel);
    ...
    private void createAnalyticsPanel() {
        analyticsPanel = new JPanel(new GridLayout(2, 1, 10, 10));
        analyticsPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));

        // Create charts
        pieChartPanel = createSeverityPieChart();
        barChartPanel = createMonthlyBarChart();

        // Set preferred sizes
        Dimension chartSize = new Dimension(500, 300);
        pieChartPanel.setPreferredSize(chartSize);
        barChartPanel.setPreferredSize(chartSize);

        // Add charts to panel
        analyticsPanel.add(pieChartPanel);
        analyticsPanel.add(barChartPanel);
    }
    ...
    public void refreshCharts() {
        if (pieChartPanel != null && barChartPanel != null) {
            analyticsPanel.remove(pieChartPanel);
            analyticsPanel.remove(barChartPanel);
            pieChartPanel = createSeverityPieChart();
            barChartPanel = createMonthlyBarChart();
            analyticsPanel.add(pieChartPanel);
            analyticsPanel.add(barChartPanel);
    ...
    private void createStatsPanel() {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            // Total Hazards
            addStatCard(statsPanel, "Total Hazards", getTotalCount(conn, "accidents"), new Color(51, 153, 255));
    ...
    private void updateAccidentStatus(int accidentId, String newStatus) {
        try (Connection conn = DatabaseConnection.getConnection()) {
